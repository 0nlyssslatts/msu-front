...
const ScheduleModalContent: React.FC<{ tasks; event: Event; onClose: () => void }> = ({ tasks, event, onClose }) => {
  ...
  const toggleDescription = (id: number) => {
    setOpenIds(prev => prev.includes(id) ? prev.filter(pid => pid !== id) : [...prev, id]);
  };
        ...
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <div key={task.id} className={styles.taskItem}>
              ...
              >
                {openIds.includes(task.id) ? 'Скрыть описание' : 'Показать описание'}
              </button>
      ...
      <div>
        ...
        <Modal
          ...
          style={{
            ...
            content: { margin: "auto", height: "auto", padding: "20px", borderRadius: "8px", backgroundColor: "#fff", boxShadow: "0 4px 6px rgba(0, 0, 0, 0.1)" }
          }}
        >
          {renderModalContent()}
        </Modal>
